...
function Countdown({ scheduledAt }) {
  ...
  return (
    <span className="text-xs font-mono text-[#1a3c6e] bg-blue-50 px-2 py-0.5 rounded-full">
      {h > 0 ? `${h}h ` : ''}{m}m {s}s
    </span>
  )
}
  ...
  if (activeSession) {
    return (
      <div className="fixed inset-0 z-50 bg-[#0a0a0a] flex flex-col">
        {/* Live Header */}
        <div className="bg-[#1a3c6e] text-white px-4 py-2.5 flex items-center justify-between flex-shrink-0">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="w-10 h-10 rounded-xl bg-[#1a3c6e] flex items-center justify-center">
              <Video size={20} className="text-[#d4a017]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold font-serif text-[#1a3c6e]">Golden Classroom</h2>
        ...
        <button
          onClick={openCreate}
          className="bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-5 py-2.5 rounded-xl flex items-center gap-2 transition-colors shadow-md"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Sessions', value: sessions.length, icon: <LayoutGrid size={18} />, color: 'text-[#1a3c6e]', bg: 'bg-blue-50' },
          ...
          {[['all', 'All Sessions'], ['scheduled', 'Upcoming'], ['live', 'Live'], ['ended', 'Ended']].map(([val, label]) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold transition-colors ${
                statusFilter === val ? 'bg-[#1a3c6e] text-white' : 'text-gray-500 hover:bg-gray-100'
              }`}
            >
              {label}
                ...
                <div key={session.id} className={`px-6 py-4 flex items-center gap-4 hover:bg-gray-50 transition-colors ${isLive ? 'bg-red-50/40' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-bold text-[#1a3c6e] text-sm">{session.title}</p>
                    ...
                    {canStart && (
                      <>
                        <button
                          ...
                          className="text-gray-400 hover:text-[#1a3c6e] p-1.5 rounded-lg hover:bg-gray-100 transition-colors"
                          title="Edit"
                        >
                          <Edit3 size={15} />
                        </button>
                        <button
                          ...
                          className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-4 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-sm"
                        >
                    ...
                    {isLive && (
                      <>
                        <button
                          ...
                          className="bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-4 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors"
                        >
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
            <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
              ...
              <button onClick={() => { setShowForm(false); setEditingSession(null); resetForm() }} className="hover:text-[#d4a017] transition-colors">
                <X size={22} />
              </button>
            </div>
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              {/* Title */}
              <div>
                <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Session Title <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Class <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Subject</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Date <span className="text-red-500">*</span></label>
                  <input type="date" value={form.date}
                    onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Time <span className="text-red-500">*</span></label>
              ...
              {/* Duration */}
              <div>
                <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Duration (minutes)</label>
              ...
              {/* Description */}
              <div>
                <label className="block text-sm font-bold text-[#1a3c6e] mb-1.5">Description (optional)</label>
                <textarea value={form.description}
                  onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  rows={3}
                  placeholder="What will you cover in this class?"
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 resize-none" />
              </div>

              <div className="bg-blue-50 rounded-xl p-3 flex items-start gap-2">
                <AlertCircle size={14} className="text-[#1a3c6e] mt-0.5 flex-shrink-0" />
