          ...
          {sidebarOpen && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-pink-100">Parent Portal</p>
              </div>
            </div>
          )}
          ...
          {menuItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 transition-colors ${
                activeMenu === item.id ? 'bg-[#ff00c8] text-[#e600a9] font-bold' : 'hover:bg-purple-900 text-purple-200'
              }`}
            >
              {item.icon}
            ...
            <div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {[
                  ...
                  { label: 'Assignments', value: assignments.length, color: 'bg-[#e600a9]', textColor: 'text-pink-100' },
                  { label: 'Avg Grade', value: 'A', color: 'bg-[#0f6e56]', textColor: 'text-green-200' },
                  { label: 'Messages', value: messages.length, color: 'bg-[#ff00c8]', textColor: 'text-[#e600a9]/80' },
                ].map((stat, index) => (
                    ...
                    <div key={student.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <span className="inline-block bg-[#ff00c8] text-[#e600a9] text-xs font-bold px-3 py-1 rounded-full mt-1">{student.studentId}</span>
                        </div>
                      </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {[
                  ...
                  ['Late', attendanceSummary.late, 'bg-[#ff00c8]', 'text-[#e600a9]/80'],
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {attendanceLoading ? (
                      ...
                      <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fff0fb]'}>
                      ...
                      <div className="flex items-start justify-between flex-wrap gap-4">
                        ...
                        <div className="flex-1 min-w-[200px]">
                          <div className="grid grid-cols-3 gap-2">
                            {['English', 'Maths', 'Science', 'Computing', 'RME', 'History'].map(subject => {
                              ...
                              return (
                                <div key={subject} className="bg-[#fff0fb] rounded-lg p-2 text-center">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <button
                            ...
                            className="bg-[#ff00c8] hover:bg-[#ff66dc] text-[#e600a9] font-bold px-6 py-3 rounded-xl transition-colors text-sm"
                          >
                            Download PDF
                          </button>
                        </div>
                ...
                {assignments.map((assignment, index) => (
                  <div key={index} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <span className={`inline-block text-xs font-bold px-3 py-1 rounded-full mb-3 ${
                      assignment.status === 'Submitted' ? 'bg-green-100 text-green-700' : 'bg-pink-100 text-[#8a0065]'
                    }`}>
                      {assignment.status}
                    </span>
                        ...
                        <tr key={payment.id} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fff0fb]'}>
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                              ...
                              payment.status === 'partial' ? 'bg-pink-100 text-[#8a0065]' :
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="bg-[#e600a9] text-white p-6 rounded-t-2xl flex items-center justify-between">
              <div>
                ...
                <p className="text-pink-100 text-sm">{viewingResult.academicYear} | {viewingResult.term}</p>
              </div>
              <button onClick={() => setViewingResult(null)} className="hover:text-[#ff00c8] transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6">
              ...
              {/* Gold bar */}
              <div className="h-1 bg-[#ff00c8] rounded-full mb-6"></div>

              {/* Student Info */}
              <div className="bg-blue-50 rounded-xl p-4 mb-6 grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-gray-500">Student Name</p>
                  <p className="font-bold text-[#e600a9]">{viewingResult.student?.firstName} {viewingResult.student?.lastName}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Student ID</p>
                  <p className="font-bold text-[#e600a9]">{viewingResult.student?.studentId}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Class</p>
                  <p className="font-bold text-[#e600a9]">{viewingResult.gradeLevel}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Gender</p>
                  <p className="font-bold text-[#e600a9]">{viewingResult.student?.gender}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {['English', 'Maths', 'Science', 'Computing', 'RME', 'History', 'Ewe', 'French', 'UC MAS'].map((subject, index) => {
                      ...
                      return (
                        <tr key={subject} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fff0fb]'}>
                          <td className="px-4 py-3 font-bold text-[#e600a9]">{subject}</td>
                    ...
                    {/* Grand Total Row */}
                    <tr className="bg-[#ff00c8]">
                      <td colSpan="5" className="px-4 py-3 font-bold text-[#e600a9]">Grand Total</td>
                      <td className="px-4 py-3 text-center font-bold text-[#e600a9]">
              ...
              <div className="bg-blue-50 rounded-xl p-4 mb-6">
                <p className="text-sm font-bold text-[#e600a9] mb-1">Class Teacher's Remarks</p>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-[#ff00c8] hover:bg-[#ff66dc] text-[#e600a9] font-bold py-3 rounded-xl transition-colors"
                >
                  Download PDF
                </button>
                <button
                  onClick={() => setViewingResult(null)}
                  className="flex-1 bg-[#e600a9] hover:bg-[#ff33c7] text-white font-bold py-3 rounded-xl transition-colors"
                >
                  Close
                </button>
              </div>
