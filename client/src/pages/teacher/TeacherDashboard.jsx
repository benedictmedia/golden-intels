...
import {
  SUBJECTS, calculateGrandTotal, getNormalizedScores, getRemarksText,
  getSubjectScore, getSubjectTotal, normalizeSubjectName
} from '../../utils/subjects'
...
export default function TeacherDashboard() {
  ...
  const [newAssignment, setNewAssignment] = useState({
    title: '', subject: '', dueDate: '', dueTime: '', description: '', gradeLevel: 'Year 1'
  })
  ...
  const [newQuiz, setNewQuiz] = useState({
    title: '', subject: '', gradeLevel: 'Year 1', dueDate: '', dueTime: '',
    durationMinutes: 30,
    questions: [{ prompt: '', type: 'multiple-choice', options: ['', '', '', ''], answer: '' }],
    published: false
  })
  ...
  // ─── Helpers ───────────────────────────────────────────────────────────────
  const normalizeSubjectKey = (v) => normalizeSubjectName(v).toLowerCase()
  ...
  const canonicalizeClass = (v) => {
    const nv = normalizeClassKey(v)
    if (!nv) return ''
    return classes.find(c => normalizeClassKey(c) === nv) || String(v ?? '').trim()
  }
  const matchesClass = (gradeLevel, targetClass) =>
    normalizeClassKey(gradeLevel) === normalizeClassKey(targetClass)
  ...
  const normalizedTeacherSubjectKeys = new Set(
    (teacherSubjectOptions || []).map(normalizeSubjectKey)
  )

  // ─── FIXED: teacher identity anchors ──────────────────────────────────────
  // Always use the authenticated user's email as the ownership key.
  ...
  useEffect(() => {
    if (['classes', 'attendance', 'gradebook'].includes(activeMenu)) {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/attendance`, {
      headers, params: { date: attendanceDate, gradeLevel: activeClass }
    })
      .then(res => {
      ...
      .catch(() => setAttendanceError('Unable to load attendance for this date.'))
    ...
    axios.get(`${API_URL}/api/results/student/${gradebookStudent}`, { headers })
      .then(res => {
        const match = res.data.find(r =>
          matchesClass(r.gradeLevel, gradebookClass) &&
          r.academicYear === gradebookYear &&
          r.term === gradebookTerm
        )
  ...
  useEffect(() => {
    ...
    // Load ALL submission records, then filter by this teacher's email below
  ...
  useEffect(() => {
    ...
    if (!classOptions.some(c => matchesClass(activeClass, c))) setActiveClass(classOptions[0])
  }, [classOptions, activeClass])

  useEffect(() => {
    if (!classOptions.length) return
    if (!classOptions.some(c => matchesClass(gradebookClass, c))) {
  ...
  // ─── FIXED: Strict teacher-scoped LMS item ownership ──────────────────────
  // Only items whose teacherEmail exactly matches this teacher are shown / editable
  const ownsLmsItem = (item) => item.teacherEmail === teacherEmail

  const teacherAssignments = assignments.filter(i => ownsLmsItem(i) && matchesAcademicContext(i))
  const teacherLessons = lessons.filter(i => ownsLmsItem(i) && matchesAcademicContext(i))
  const teacherQuizzes = quizzes.filter(i => ownsLmsItem(i) && matchesAcademicContext(i))

  // ─── FIXED: Submission records strictly scoped to this teacher ─────────────
  // A submission belongs to this teacher only if the assignment/quiz was created by this teacher.
  const teacherSubmissionRecords = submissionRecords.filter(record =>
    record.teacherEmail === teacherEmail &&
    matchesAcademicContext(record)
  )

  const getResponsesForItem = (item, type) =>
    teacherSubmissionRecords.filter(
      r => r.type === type && String(r.itemId) === String(item.id)
    )
  ...
  const isClassTeacherForSelectedClass = classTeacherClassOptions.some(
    c => matchesClass(gradebookClass, c)
  )
  const visibleSubjects = isClassTeacherForSelectedClass
    ? SUBJECTS
    : SUBJECTS.filter(s => normalizedTeacherSubjectKeys.has(normalizeSubjectKey(s)))

  const totalAllSubjects = calculateGrandTotal(subjectScores, visibleSubjects)
  const subjectsCompleted = visibleSubjects.filter(s => {
    const sc = subjectScores[s] || {}
    return sc.classScore && sc.cat1 && sc.cat2 && sc.exam
  }).length
  const contextualSubmittedResults = submittedResults.filter(
    r => r.academicYear === selectedAcademicYear && r.term === selectedTerm
  )
  ...
  const handleGradebookSubmit = async () => {
    ...
    if (!Object.keys(payloadScores).length) {
      alert('You do not have any assigned subjects for this class. Please contact admin.')
      return
    }
    const token = localStorage.getItem('token')
    const headers = { Authorization: `Bearer ${token}` }
    try {
      if (editingResult) {
        const res = await axios.put(`${API_URL}/api/results/${editingResult.id}`, {
          scores: payloadScores, remarks: teacherRemarks, status: 'pending'
        }, { headers })
        ...
        const res = await axios.post(`${API_URL}/api/results`, {
          studentId: gradebookStudent, gradeLevel: gradebookClass,
          academicYear: gradebookYear, term: gradebookTerm,
          scores: payloadScores, remarks: teacherRemarks, submittedBy: user?.name
        }, { headers })
  ...
  const handleDownloadPDF = async (result) => {
    ...
    doc.text(`${student.dateOfBirth}`, 150, 82); doc.text(`${student.status}`, 150, 91)
    let y = 100
    ...
    SUBJECTS.forEach((subject, index) => {
      ...
      const cat1 = parseFloat(s.cat1) || 0; const cat2 = parseFloat(s.cat2) || 0
      const exam = parseFloat(s.exam) || 0; const wExam = (exam / 100) * 50
      const total = getSubjectTotal(s); grandTotal += total
      ...
      doc.text(cat2.toString(), 126, y + 5.5); doc.text(wExam.toFixed(2), 150, y + 5.5)
      doc.text(total.toFixed(2), 170, y + 5.5)
      const gc = getGradeColor(total); doc.setFont('helvetica', 'bold'); doc.setTextColor(gc[0], gc[1], gc[2])
      doc.text(total > 0 ? getGrade(total) : '-', 188, y + 5.5); y += 8
    })
    ...
    doc.text('Grand Total', 15, y + 5.5); doc.text(`${grandTotal.toFixed(2)} / 1100`, 155, y + 5.5); y += 10
    const _attHeaders = { Authorization: `Bearer ${localStorage.getItem('token')}` }
    let attendanceSummary = null
    try {
      const ar = await axios.get(`${API_URL}/api/attendance/summary/${result.studentId}`, { headers: _attHeaders })
      attendanceSummary = ar.data
    } catch {}
    ...
    const clampedRemarksLines = doc.splitTextToSize(remarksText, 172).slice(0, 2)
    ...
    doc.text(clampedRemarksLines, 15, y + 9); y += remarksHeight + 4
  ...
  const saveAttendance = async () => {
    const token = localStorage.getItem('token')
    const headers = { Authorization: `Bearer ${token}` }
    const classStudents = students.filter(s => matchesClass(s.gradeLevel, activeClass))
    setAttendanceSaving(true); setAttendanceError('')
    try {
      await axios.post(`${API_URL}/api/attendance`, {
        date: attendanceDate, gradeLevel: activeClass,
        recordedBy: user?.name || 'Teacher',
        records: classStudents.map(s => ({
          studentId: s.id, status: attendance[s.id] || 'present'
        }))
      }, { headers })
    ...
    } catch { setAttendanceError('Failed to save attendance. Please try again.') }
    finally { setAttendanceSaving(false) }
  }

  const handleSubjectScore = (subject, field, value) =>
    setSubjectScores(prev => ({ ...prev, [subject]: { ...prev[subject], [field]: value } }))
  ...
  // ─── LMS handlers ──────────────────────────────────────────────────────────
  const resetLmsForm = () => {
    setEditingLmsItem(null); setLmsItemView(null)
  ...
  const handleViewLmsItem = (item, type) => {
    setShowAddAssignment(false); setShowAddLesson(false); setShowAddQuiz(false)
    setLmsItemView({ item, type })
  }

  const handleDownloadSubmissionPdf = (record) => {
    const doc = new jsPDF()
    doc.setFontSize(16); doc.text('Student Submission Report', 14, 20)
    doc.setFontSize(11)
    ...
    doc.text(`Resource: ${record.type} - ${record.title || 'N/A'}`, 14, 64)
    if (record.score != null) {
      doc.text(`Score: ${record.score} / ${record.totalQuestions ?? record.questions?.length ?? 'N/A'}`, 14, 70)
    }
    doc.text('Answers:', 14, 82); let y = 88
    ;(record.questions || []).forEach((q, idx) => {
      if (y > 270) { doc.addPage(); y = 20 }
      doc.text(`Q${idx + 1}: ${q.prompt}`, 14, y); y += 6
      doc.text(`Your: ${q.selected || 'N/A'}`, 18, y); y += 6
      doc.text(`Correct: ${q.answer || 'N/A'}`, 18, y); y += 8
    })
    doc.save(`${record.learnerName || 'student'}-${record.type}-submission.pdf`)
  }

  const getSubmissionRecordKey = (record) =>
    `${record.type}-${record.itemId}-${record.learnerEmail || record.learnerName || 'learner'}`

  // ─── FIXED: Mark handler – updates the shared localStorage correctly ───────
  const handleMarkAssignmentSubmission = (record) => {
    ...
    const updated = submissionRecords.map(item =>
      getSubmissionRecordKey(item) === targetKey
        ? {
            ...item, score: score.trim(), feedback: feedback.trim(),
            marked: true, markedAt: new Date().toISOString(),
            markedBy: user?.name || 'Teacher'
          }
        : item
    )
    setSubmissionRecords(updated)
    // Write through to localStorage immediately so parent dashboard picks it up
    window.localStorage.setItem('goldenIntelsSubmissionRecords', JSON.stringify(updated))
  }

  const handleEditLmsItem = (item, type) => {
    setEditingLmsItem({ item, type }); setLmsItemView({ item, type })
    if (type === 'assignment') {
      const pd = item.dueDate ? new Date(item.dueDate) : null
      setNewAssignment({
        title: item.title, subject: item.subject,
        dueDate: pd ? pd.toISOString().slice(0, 10) : item.dueDate || '',
        dueTime: pd ? pd.toISOString().slice(11, 16) : (item.dueTime || ''),
        description: item.description, gradeLevel: item.gradeLevel
      })
      setShowAddAssignment(true); setShowAddLesson(false); setShowAddQuiz(false)
    }
    if (type === 'lesson') {
      setNewLesson({ title: item.title, subject: item.subject, gradeLevel: item.gradeLevel, content: item.content })
      setShowAddLesson(true); setShowAddAssignment(false); setShowAddQuiz(false)
    }
    if (type === 'quiz') {
      const pd = item.dueDate ? new Date(item.dueDate) : null
      setNewQuiz({
        title: item.title, subject: item.subject, gradeLevel: item.gradeLevel,
        dueDate: pd ? pd.toISOString().slice(0, 10) : item.dueDate || '',
        dueTime: pd ? pd.toISOString().slice(11, 16) : (item.dueTime || ''),
        durationMinutes: item.durationMinutes,
        questions: item.questions?.map(q => ({
          type: q.type || 'multiple-choice', prompt: q.prompt,
          options: q.options || ['', '', '', ''], answer: q.answer
        })) || [{ prompt: '', type: 'multiple-choice', options: ['', '', '', ''], answer: '' }],
        published: item.published
      })
      setShowAddQuiz(true); setShowAddAssignment(false); setShowAddLesson(false)
    }
  }

  const handleDeleteLmsItem = (itemId, type) => {
    if (!window.confirm('Delete this item? This cannot be undone.')) return
    if (type === 'lesson') setLessons(prev => prev.filter(i => i.id !== itemId))
    if (type === 'assignment') setAssignments(prev => prev.filter(i => i.id !== itemId))
    if (type === 'quiz') setQuizzes(prev => prev.filter(i => i.id !== itemId))
    if (lmsItemView?.item?.id === itemId) setLmsItemView(null)
    if (editingLmsItem?.item?.id === itemId) resetLmsForm()
  }

  // ─── FIXED: Always stamp teacherEmail + teacherName + academicYear + term ──
  const handleAddAssignment = () => {
    if (!newAssignment.title) return
    const base = {
      ...newAssignment,
      dueDate: combineDateTime(newAssignment.dueDate, newAssignment.dueTime),
      teacherEmail,
      teacherName,
    ...
    if (editingLmsItem?.type === 'assignment') {
      setAssignments(prev => prev.map(i =>
        i.id === editingLmsItem.item.id
          ? { ...i, ...base, published: editingLmsItem.item.published }
          : i
      ))
      setEditingLmsItem(null)
    } else {
      setAssignments(prev => [{ id: Date.now(), published: false, ...base }, ...prev])
    }
    resetLmsForm(); setShowAddAssignment(false)
  }

  const handleAddLesson = () => {
    if (!newLesson.title) return
    const base = { ...newLesson, teacherEmail, teacherName, academicYear: selectedAcademicYear, term: selectedTerm }
    if (editingLmsItem?.type === 'lesson') {
      setLessons(prev => prev.map(i =>
        i.id === editingLmsItem.item.id
          ? { ...i, ...base, published: editingLmsItem.item.published }
          : i
      ))
      ...
      setLessons(prev => [{ id: Date.now(), published: false, ...base }, ...prev])
    }
    resetLmsForm(); setShowAddLesson(false)
  }
  ...
  const handleAddQuiz = () => {
    ...
    const base = {
      ...newQuiz,
      dueDate: combineDateTime(newQuiz.dueDate, newQuiz.dueTime),
      teacherEmail,
      teacherName,
      academicYear: selectedAcademicYear,
      term: selectedTerm,
    ...
    if (editingLmsItem?.type === 'quiz') {
      ...
      setQuizzes(prev => [{ id: Date.now(), published: false, ...base }, ...prev])
    }
    resetLmsForm(); setShowAddQuiz(false)
  }
  ...
  const handleQuizOptionChange = (qi, oi, value) => {
    setNewQuiz(prev => ({
      ...prev,
      questions: prev.questions.map((q, i) => {
        if (i !== qi) return q
  ...
  const removeQuizQuestion = (index) =>
    setNewQuiz(prev => ({ ...prev, questions: prev.questions.filter((_, i) => i !== index) }))

  const togglePublishItem = (itemId, type) => {
    if (type === 'lesson') setLessons(prev => prev.map(i => i.id === itemId ? { ...i, published: !i.published } : i))
    if (type === 'assignment') setAssignments(prev => prev.map(i => i.id === itemId ? { ...i, published: !i.published } : i))
    if (type === 'quiz') setQuizzes(prev => prev.map(i => i.id === itemId ? { ...i, published: !i.published } : i))
  }

  const handleLogout = () => { logout(); navigate('/') }

  const filteredStudents = students.filter(s => matchesClass(s.gradeLevel, activeClass))
  const attendanceStats = filteredStudents.reduce((stats, s) => {
    const status = attendance[s.id] || 'present'
    return { ...stats, [status]: (stats[status] || 0) + 1 }
  }, { present: 0, absent: 0, late: 0 })
  const attendanceCompletion = filteredStudents.length
    ? Math.round((Object.keys(attendance).filter(id =>
        filteredStudents.some(s => s.id.toString() === id.toString())
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <select
                ...
                onChange={e => { setSelectedAcademicYear(e.target.value); setGradebookYear(e.target.value) }}
              ...
              <select
                value={selectedTerm}
                onChange={e => { setSelectedTerm(e.target.value); setGradebookTerm(e.target.value) }}
                className="px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700 bg-white"
              >
            ...
            <div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {[
                  ...
                  { label: 'Classes', value: teacherClassOptions.length || 1, color: 'bg-blue-500', textColor: 'text-cyan-700/80' },
                ].map((stat, i) => (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredStudents.length === 0 ? (
                      <tr><td colSpan="5" className="px-6 py-8 text-center text-gray-400">No students in this class.</td></tr>
                      ...
                      <tr key={student.id} className={index % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                        <td className="px-6 py-4">
                          <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 border-2 border-[#0f6e56]">
                            {student.photo ? (
                              <img src={student.photo} alt={student.firstName} className="w-full h-full object-cover" loading="lazy" />
                            ) : (
            ...
            <div>
              <div className="flex items-start justify-between gap-4 flex-wrap mb-6">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Record daily class attendance. Saved records update parent dashboards immediately.</p>
                </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {[
                  ...
                  <div key={label} className={`${color} text-white rounded-2xl p-5 shadow-sm`}>
                    <p className={`${tc} text-xs font-bold uppercase tracking-wide`}>{label}</p>
              ...
              {classTeacherClassOptions.length === 0 && (
                <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-lg mb-4 text-sm">
                  You are not assigned as class teacher for any class yet.
                </div>
              )}
              {attendanceSaved && (
              ...
              {attendanceError && (
                <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg mb-4 text-sm">{attendanceError}</div>
              )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {attendanceLoading ? (
                      ...
                      <tr><td colSpan="3" className="px-6 py-8 text-center text-gray-400">No students in this class.</td></tr>
                    ) : filteredStudents.map((student, index) => (
                      <tr key={student.id} className={index % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="inline-flex bg-blue-100 rounded-xl p-1">
                            {[['present','Present','bg-[#0f6e56] text-white'],['absent','Absent','bg-red-500 text-white'],['late','Late','bg-blue-500 text-cyan-700']].map(([status, label, ac]) => (
                              <button key={status} type="button" onClick={() => handleAttendance(student.id, status)}
                                className={`px-4 py-2 rounded-lg text-xs font-bold transition-colors ${
                                  (attendance[student.id] || 'present') === status ? ac : 'text-gray-500 hover:text-[#0f6e56]'
                                }`}
                              >{label}</button>
                            ))}
                          </div>
                        ...
                        <td className="px-6 py-4 text-gray-500">
                          {attendanceRecords.some(r => r.studentId === student.id) ? 'Saved' : 'Pending'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <button onClick={saveAttendance} disabled={attendanceSaving || filteredStudents.length === 0}
                className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-8 py-3 rounded-xl disabled:opacity-50"
              >
          ...
          {activeMenu === 'gradebook' && (
            <div>
              ...
              <div className="flex gap-3 mb-6">
                {['enter', 'submitted'].map(tab => (
                  <button key={tab} onClick={() => setActiveGradebookTab(tab)}
                    className={`px-6 py-2 rounded-full text-sm font-bold transition-colors ${
                      activeGradebookTab === tab ? 'bg-[#0f6e56] text-white' : 'bg-white text-[#0f6e56] border border-[#0f6e56]'
                    }`}
                  >
                    {tab === 'enter' ? 'Enter Performance' : (
                      <>Submitted Results <span className="ml-2 bg-blue-500 text-cyan-700 text-xs font-bold px-2 py-0.5 rounded-full">{contextualSubmittedResults.length}</span></>
                    )}
                  </button>
                ))}
              </div>
              ...
              {gradebookSubmitted && (
                <div className="bg-green-50 border border-green-200 text-green-600 px-4 py-3 rounded-lg mb-4 text-sm">
                  {editingResult ? 'Result updated successfully!' : 'Performance data submitted for admin review!'}
                </div>
              )}
              ...
              {activeGradebookTab === 'submitted' && (
                <div>
                  {contextualSubmittedResults.length === 0 ? (
                    <div className="bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">No results submitted yet.</div>
                  ) : (
                          ...
                          <div className="flex items-center justify-between flex-wrap gap-4">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                                ...
                                result.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-cyan-700'
                              }`}>{result.status}</span>
                              <button onClick={() => handleEditResult(result)}
                                className="bg-blue-500 hover:bg-blue-300 text-cyan-700 font-bold px-4 py-2 rounded-lg text-sm">Edit</button>
                              <button onClick={() => handleDownloadPDF(result)}
                                className="bg-blue-600 hover:bg-blue-400 text-white font-bold px-4 py-2 rounded-lg text-sm">Download PDF</button>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                    <div>
                      ...
                      <select value={gradebookClass} onChange={e => { setGradebookClass(e.target.value); setGradebookStudent('') }}
                    ...
                    <div>
                      ...
                      <select value={gradebookYear} onChange={e => { setGradebookYear(e.target.value); setSelectedAcademicYear(e.target.value) }}
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700"
                      >
                        {academicYears.map(y => <option key={y} value={y}>{y}</option>)}
                      </select>
                    ...
                    <div>
                      ...
                      <select value={gradebookTerm} onChange={e => { setGradebookTerm(e.target.value); setSelectedTerm(e.target.value) }}
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700"
                      >
                        {terms.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    ...
                    <div>
                      ...
                      <select value={gradebookStudent} onChange={e => setGradebookStudent(e.target.value)}
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700"
                      >
                        <option value="">Choose a student...</option>
                        {students.filter(s => matchesClass(s.gradeLevel, gradebookClass)).map(s => (
                  ...
                  {gradebookLoading && <p className="text-sm text-gray-500 mb-4">Loading current result...</p>}
                  {gradebookError && <p className="text-sm text-amber-600 mb-4">{gradebookError}</p>}

                  <div className="space-y-6 mb-8">
                    {visibleSubjects.map(subject => {
                      const sc = subjectScores[subject] || { classScore: '', cat1: '', cat2: '', exam: '' }
                      const editable = normalizedTeacherSubjectKeys.has(normalizeSubjectKey(subject))
                      const classScore = parseFloat(sc.classScore) || 0
                      const cat1 = parseFloat(sc.cat1) || 0
                      const cat2 = parseFloat(sc.cat2) || 0
                      ...
                      return (
                        <div key={subject} className="bg-blue-50 rounded-2xl p-6 border border-gray-100">
                          <div className="flex items-center justify-between gap-4 mb-4">
                            ...
                            {!editable && <span className="text-xs font-bold px-3 py-1 rounded-full bg-gray-200 text-gray-700">Read-only</span>}
                          </div>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                            {[
                              { label: 'Class Score (10%)', field: 'classScore', max: 10 },
                              { label: 'CAT 1 (20%)', field: 'cat1', max: 20 },
                              { label: 'CAT 2 (20%)', field: 'cat2', max: 20 },
                              { label: 'Exam Score (50%)', field: 'exam', max: 100, note: `Weighted: ${wExam.toFixed(2)}` },
                            ].map(({ label, field, max, note }) => (
                              <div key={field}>
                                <label className="block text-xs font-bold text-gray-500 mb-1">{label}</label>
                                <input type="number" min="0" max={max} value={sc[field]}
                                  onChange={e => handleSubjectScore(subject, field, e.target.value)}
                                  readOnly={!editable}
                                ...
                                />
                                <p className="text-xs text-gray-400 mt-1">{note || `Max: ${max}`}</p>
                              </div>
                            ))}
                          </div>
                  ...
                  <div className="mb-8">
                    ...
                    <p className="text-xs text-gray-400 mb-2">
                      {isClassTeacherForSelectedClass
                        ? "Provide constructive feedback on the student's overall performance."
                    ...
                    <textarea value={teacherRemarks} onChange={e => setTeacherRemarks(e.target.value)}
                      rows={4} readOnly={!isClassTeacherForSelectedClass}
                      placeholder={isClassTeacherForSelectedClass ? 'Enter remarks here...' : 'Read-only for non-class teachers.'}
                  ...
                  <button onClick={handleGradebookSubmit} disabled={!gradebookStudent}
                    className="w-full bg-[#0f6e56] hover:bg-[#085041] text-white font-bold py-4 rounded-xl disabled:opacity-50 text-lg"
                  >
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div className="flex flex-wrap gap-3">
                  {['resources', 'assessments', 'submissions'].map(v => (
                    <button key={v} onClick={() => setLmsView(v)}
                      className={`px-4 py-2 rounded-full text-sm font-bold transition ${
                        lmsView === v ? 'bg-[#0f6e56] text-white' : 'bg-white text-[#0f6e56] border border-[#0f6e56]'
                      }`}
                    >
                      {v === 'submissions' ? 'Responses' : v.charAt(0).toUpperCase() + v.slice(1)}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {[
                  { label: 'Published Lessons', value: teacherLessons.filter(i => i.published).length },
                  { label: 'Published Assignments', value: teacherAssignments.filter(i => i.published).length },
                  { label: 'Published Quizzes', value: teacherQuizzes.filter(i => i.published).length },
                ].map(s => (
                  <div key={s.label} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 text-center">
                    <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-3">{s.label}</p>
                    <p className="text-3xl font-bold text-[#0f6e56]">{s.value}</p>
                  </div>
                ))}
              </div>
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-6">
                  ...
                  {lmsItemView.type === 'quiz' && (
                    <div className="space-y-3">
                      ...
                      <div className="space-y-4">
                        {lmsItemView.item.questions.map((q, i) => (
                          <div key={i} className="bg-slate-50 rounded-2xl p-4">
                            <p className="font-bold text-[#0f6e56] mb-2">Q{i + 1}. {q.prompt}</p>
                            <div className="grid gap-2">
                              {(q.options || []).map((opt, oi) => (
                                <div key={oi} className={`text-sm ${opt === q.answer ? 'font-bold text-green-700' : 'text-gray-700'}`}>
                                  {opt === q.answer ? '✔ ' : ''}{opt}
                                </div>
                              ))}
                            </div>
                            <p className="text-xs text-gray-400 mt-2">Answer: {q.answer || 'N/A'}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      {getResponsesForItem(lmsItemView.item, lmsItemView.type).length === 0 ? (
                        ...
                        <div className="overflow-hidden rounded-2xl border border-gray-100">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {getResponsesForItem(lmsItemView.item, lmsItemView.type).map((record, i) => (
                                <tr key={`${record.itemId}-${record.learnerEmail}-${i}`} className="border-t border-gray-100">
                                  ...
                                  <td className="px-4 py-3">
                                    <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                                      record.marked ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                    }`}>
                                      {record.marked ? 'Marked' : 'Awaiting mark'}
                                    </span>
                                  </td>
                                </tr>
                              ))}
                ...
                <div>
                  <div className="flex flex-wrap gap-3 mb-6">
                    <button onClick={() => setShowAddLesson(true)}
                      className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-6 py-2 rounded-lg text-sm">
                      + New Lesson
                    </button>
                  </div>
                  {showAddLesson && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                          ...
                          {teacherSubjectOptions.length > 0 ? (
                            <select value={newLesson.subject} onChange={e => setNewLesson({ ...newLesson, subject: e.target.value })}
                              ...
                              {teacherSubjectOptions.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                          ) : (
                            <input type="text" value={newLesson.subject} onChange={e => setNewLesson({ ...newLesson, subject: e.target.value })}
                              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700" />
                          )}
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#0f6e56] mb-2">Grade Level</label>
                          <select value={newLesson.gradeLevel} onChange={e => setNewLesson({ ...newLesson, gradeLevel: e.target.value })}
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700">
                            {teacherClassOptions.map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        </div>
                        <div className="md:col-span-2">
                          ...
                          <textarea value={newLesson.content} onChange={e => setNewLesson({ ...newLesson, content: e.target.value })}
                            rows={5} placeholder="Write your lesson content here..."
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700" />
                        </div>
                      </div>
                      <div className="flex gap-4 mt-6">
                        <button onClick={handleAddLesson}
                          className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-8 py-3 rounded-xl">Save Lesson</button>
                        <button onClick={() => setShowAddLesson(false)}
                          className="bg-blue-100 hover:bg-gray-200 text-gray-700 font-bold px-8 py-3 rounded-xl">Cancel</button>
                      </div>
                    </div>
                  )}
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {teacherLessons.length === 0 ? (
                      <div className="col-span-3 bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">
                        No lessons yet. Click "+ New Lesson" to publish learning resources.
                      </div>
                      ...
                      <div key={lesson.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between gap-4 mb-4">
                          ...
                          <button onClick={() => togglePublishItem(lesson.id, 'lesson')}
                            className={`text-xs font-bold px-3 py-1 rounded-full ${lesson.published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        ...
                        <p className="text-xs font-bold text-gray-400 mb-3">{lesson.academicYear} | {lesson.term}</p>
                        <p className="text-sm text-gray-600 leading-relaxed line-clamp-3">{lesson.content}</p>
                        <div className="flex flex-wrap gap-2 mt-4">
                          <button onClick={() => handleViewLmsItem(lesson, 'lesson')}
                            className="bg-[#0f6e56] hover:bg-[#085041] text-white text-xs font-bold px-3 py-2 rounded-full">View</button>
                          <button onClick={() => handleEditLmsItem(lesson, 'lesson')}
                            className="bg-white border border-[#0f6e56] text-[#0f6e56] text-xs font-bold px-3 py-2 rounded-full">Edit</button>
                          <button onClick={() => handleDeleteLmsItem(lesson.id, 'lesson')}
                            className="bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold px-3 py-2 rounded-full">Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div className="flex flex-wrap gap-3 mb-6">
                    <button onClick={() => setShowAddAssignment(true)}
                      className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-6 py-2 rounded-lg text-sm">+ New Assignment</button>
                    <button onClick={() => setShowAddQuiz(true)}
                      className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-6 py-2 rounded-lg text-sm">+ New Quiz</button>
                  </div>
                    ...
                    <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {[
                          ...
                          <div key={key}>
                            <label className="block text-sm font-bold text-[#0f6e56] mb-2">{label}</label>
                            <input type={type} value={newAssignment[key]}
                              onChange={e => setNewAssignment({ ...newAssignment, [key]: e.target.value })}
                              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700" />
                          </div>
                        ))}
                        <div>
                          ...
                          {teacherSubjectOptions.length > 0 ? (
                            ...
                            <input type="text" value={newAssignment.subject}
                              onChange={e => setNewAssignment({ ...newAssignment, subject: e.target.value })}
                              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700" />
                          )}
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#0f6e56] mb-2">Grade Level</label>
                          <select value={newAssignment.gradeLevel}
                        ...
                        <div>
                          ...
                          <input type="time" value={newAssignment.dueTime}
                      ...
                      <div className="flex gap-4 mt-6">
                        <button onClick={handleAddAssignment}
                          className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-8 py-3 rounded-xl">Save Assignment</button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    {teacherAssignments.length === 0 ? (
                      <div className="col-span-2 bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">
                        No assignments yet. Publish assignments to make them visible to learners.
                      </div>
                      ...
                      <div key={assignment.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between gap-4 mb-4">
                          ...
                          <button onClick={() => togglePublishItem(assignment.id, 'assignment')}
                            className={`text-xs font-bold px-3 py-1 rounded-full ${assignment.published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        ...
                        <p className="text-xs font-bold text-gray-400 mb-2">{assignment.academicYear} | {assignment.term}</p>
                        <p className="text-sm text-gray-600 mb-3">{assignment.description}</p>
                        <p className="text-xs text-gray-400 mb-4">Due: {formatDateTime(assignment.dueDate)}</p>
                        <div className="flex flex-wrap gap-2 mt-2">
                          <button onClick={() => handleViewLmsItem(assignment, 'assignment')}
                            className="bg-[#0f6e56] hover:bg-[#085041] text-white text-xs font-bold px-3 py-2 rounded-full">View</button>
                          <button onClick={() => handleEditLmsItem(assignment, 'assignment')}
                            className="bg-white border border-[#0f6e56] text-[#0f6e56] text-xs font-bold px-3 py-2 rounded-full">Edit</button>
                          <button onClick={() => handleDeleteLmsItem(assignment.id, 'assignment')}
                            className="bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold px-3 py-2 rounded-full">Delete</button>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        ...
                        <div>
                          <label className="block text-sm font-bold text-[#0f6e56] mb-2">Subject</label>
                          {teacherSubjectOptions.length > 0 ? (
                            <select value={newQuiz.subject} onChange={e => setNewQuiz({ ...newQuiz, subject: e.target.value })}
                              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700">
                              <option value="">Choose subject</option>
                              {teacherSubjectOptions.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                        ...
                        <div>
                          <label className="block text-sm font-bold text-[#0f6e56] mb-2">Grade Level</label>
                          <select value={newQuiz.gradeLevel} onChange={e => setNewQuiz({ ...newQuiz, gradeLevel: e.target.value })}
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700">
                            {teacherClassOptions.map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        </div>
                        ...
                        <div>
                          <label className="block text-sm font-bold text-[#0f6e56] mb-2">Due Time</label>
                          <input type="time" value={newQuiz.dueTime} onChange={e => setNewQuiz({ ...newQuiz, dueTime: e.target.value })}
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700" />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#0f6e56] mb-2">Duration (minutes)</label>
                          <input type="number" min="5" value={newQuiz.durationMinutes}
                          ...
                          <div key={index} className="bg-blue-50 rounded-2xl p-5 border border-gray-200">
                            <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <select value={question.type || 'multiple-choice'}
                                  onChange={e => handleQuizQuestionChange(index, 'type', e.target.value)}
                                  className="px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700">
                            ...
                            <input type="text" value={question.prompt}
                              onChange={e => handleQuizQuestionChange(index, 'prompt', e.target.value)}
                              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700 mb-4" />
                            {(question.type === 'multiple-choice' || question.type === 'dropdown') ? (
                              <>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                  {question.options.map((opt, oi) => (
                                    <div key={oi}>
                                      <label className="block text-sm font-bold text-[#0f6e56] mb-2">Option {oi + 1}</label>
                                      <input type="text" value={opt}
                                        onChange={e => handleQuizOptionChange(index, oi, e.target.value)}
                                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700" />
                                    </div>
                                  ))}
                                </div>
                                <label className="block text-sm font-bold text-[#0f6e56] mb-2">Correct Answer</label>
                                <select value={question.answer} onChange={e => handleQuizQuestionChange(index, 'answer', e.target.value)}
                                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0f6e56] text-gray-700">
                              ...
                              <p className="text-xs text-gray-500 mt-2">This response type will be marked manually.</p>
                            ) : (
                              <>
                                <label className="block text-sm font-bold text-[#0f6e56] mb-2">Correct Answer</label>
                      ...
                      <button type="button" onClick={addQuizQuestion}
                        className="bg-white border border-[#0f6e56] text-[#0f6e56] font-bold px-6 py-3 rounded-xl mt-4">
                        Add Question
                      </button>
                      <div className="flex gap-4 mt-6">
                        <button onClick={handleAddQuiz}
                          className="bg-[#0f6e56] hover:bg-[#085041] text-white font-bold px-8 py-3 rounded-xl">Save Quiz</button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {teacherQuizzes.length === 0 ? (
                      <div className="col-span-2 bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">
                        No quizzes yet. Publish quizzes to give learners online practice.
                      </div>
                      ...
                      <div key={quiz.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between gap-4 mb-4">
                          ...
                          <button onClick={() => togglePublishItem(quiz.id, 'quiz')}
                            className={`text-xs font-bold px-3 py-1 rounded-full ${quiz.published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                            {quiz.published ? 'Published' : 'Draft'}
                          </button>
                        </div>
                        <h3 className="text-lg font-bold text-[#0f6e56] mb-2">{quiz.title}</h3>
                        <p className="text-sm text-gray-500 mb-2">{quiz.subject}</p>
                        <p className="text-xs font-bold text-gray-400 mb-2">{quiz.academicYear} | {quiz.term}</p>
                        <p className="text-sm text-gray-600 mb-3">Due {formatDateTime(quiz.dueDate)} · {quiz.durationMinutes} minutes</p>
                        <div className="flex flex-wrap gap-2 mt-4">
                          <button onClick={() => handleViewLmsItem(quiz, 'quiz')}
                            className="bg-[#0f6e56] hover:bg-[#085041] text-white text-xs font-bold px-3 py-2 rounded-full">View</button>
                          <button onClick={() => handleEditLmsItem(quiz, 'quiz')}
                            className="bg-white border border-[#0f6e56] text-[#0f6e56] text-xs font-bold px-3 py-2 rounded-full">Edit</button>
                          <button onClick={() => handleDeleteLmsItem(quiz.id, 'quiz')}
                            className="bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold px-3 py-2 rounded-full">Delete</button>
                        </div>
                      </div>
                    ))}
                  ...
                  {teacherSubmissionRecords.length === 0 ? (
                    ...
                    <div className="space-y-6">
                      {teacherSubmissionRecords.map((record, index) => (
                        <div key={`${record.type}-${record.itemId}-${record.learnerEmail}-${index}`}
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                Student: {record.learnerName || 'Unknown'} · {record.learnerEmail || 'No email'}
                              </p>
                              {record.studentId && <p className="text-xs text-gray-400">ID: {record.studentId}</p>}
                            </div>
                            <div className="text-sm text-gray-500 text-right shrink-0">
                              <p>Submitted: {formatDateTime(record.submittedAt)}</p>
                              <p>Device: {(record.device || 'Unknown').slice(0, 30)}</p>
                          ...
                          <div className="bg-slate-50 rounded-2xl p-4 mb-4">
                            <p className="text-sm font-bold text-[#0f6e56] mb-3">Answered Questions</p>
                            <div className="space-y-3">
                              {(record.questions || []).map((q, idx) => {
                                const needsManual = ['short-answer', 'paragraph'].includes(q.type)
                                const isCorrect = !needsManual &&
                                  q.answer != null &&
                                  String(q.selected || '').trim().toLowerCase() === String(q.answer).trim().toLowerCase()
                                return (
                                  <div key={idx} className={`rounded-2xl p-3 border ${
                                    needsManual ? 'bg-yellow-50 border-yellow-200' :
                                    isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                                  }`}>
                                    <p className="font-bold text-[#0f6e56]">Q{idx + 1}. {q.prompt}</p>
                                    <p className="text-sm text-gray-700 mt-1">
                                      Answer: {q.selected || 'No answer'}
                                    </p>
                                    {!needsManual && (
                                      <p className={`text-xs font-bold mt-1 ${isCorrect ? 'text-green-700' : 'text-red-600'}`}>
                          ...
                          {record.marked && (
                            <div className="bg-green-50 border border-green-200 rounded-2xl p-4 mb-4">
                              ...
                              <p className="text-sm text-gray-700 mt-1">
                                Score: {record.score || 'Marked'}{record.totalQuestions ? ` / ${record.totalQuestions}` : ''}
                              </p>
                              {record.feedback && <p className="text-sm text-gray-700 mt-1">Feedback: {record.feedback}</p>}
                              <p className="text-xs text-gray-500 mt-2">
                                Marked by {record.markedBy || 'Teacher'} on {formatDateTime(record.markedAt)}
                              </p>
                            </div>
                          )}

                          <div className="flex flex-wrap gap-3">
                            <button onClick={() => handleDownloadSubmissionPdf(record)}
                              className="bg-[#0f6e56] hover:bg-[#085041] text-white text-xs font-bold px-4 py-2 rounded-full">
                              Download PDF
                            </button>
                            {['assignment', 'quiz'].includes(record.type) && (
                              <button onClick={() => handleMarkAssignmentSubmission(record)}
                                className="bg-blue-600 hover:bg-blue-400 text-white text-xs font-bold px-4 py-2 rounded-full">
