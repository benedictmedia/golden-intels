...
export default function AdminDashboard() {
 ...
 const [adminEditRemarks, setAdminEditRemarks] = useState('')
  ...
  const handleApproveApplication = async (id) => {
    try {
      const res = await axios.put(`/api/admissions/${id}/approve`)
  ...
  const handleRejectApplication = async (id) => {
    try {
      const res = await axios.put(`/api/admissions/${id}/reject`)
  ...
  const [newsForm, setNewsForm] = useState({
    title: '', content: '', category: 'General', type: 'news',
    videoUrl: '', eventDate: '', venue: ''
  })
  ...
  const handleNewsImageChange = (e) => {
    ...
    setNewsImages(files)
    setNewsPreviews(files.map(f => URL.createObjectURL(f)))
  }
  ...
  const handleEditNews = async () => {
    try {
      ...
      const res = await axios.put(`/api/news/${editingNews.id}`, data, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
  ...
  const [staffForm, setStaffForm] = useState({
    name: '', role: '', department: '', subject: '', bio: '', email: '', phone: '', category: 'teaching'
  })
  ...
  const handleStaffPhotoChange = (e) => {
    ...
    if (file) { setStaffPhoto(file); setStaffPhotoPreview(URL.createObjectURL(file)) }
  }
  ...
  const handleEditStaff = async () => {
    try {
      ...
      Object.entries(staffForm).forEach(([key, value]) => data.append(key, value))
      if (staffPhoto) data.append('photo', staffPhoto)
      const res = await axios.put(`/api/staff/${editingStaff.id}`, data, { headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  const handleEditGalleryItem = async () => {
    try {
      ...
      setGalleryForm({ title: '', description: '', category: 'Events' })
      setGalleryImages([])
      setGalleryPreviews([])
    } catch (err) {
  ...
  const handleAddGalleryItem = async () => {
    if (!galleryForm.title || galleryImages.length === 0) {
  ...
  const handleDeleteToken = async (id) => {
    ...
    try {
      await axios.delete(`/api/admission-tokens/${id}`)
  ...
  const filteredFeePayments = feeClassFilter === 'All'
    ? feePayments
    : feePayments.filter(p => p.student?.gradeLevel === feeClassFilter)
  ...
  const handleAddPayment = async () => {
    if (!paymentForm.studentId || !paymentForm.month || !paymentForm.year || !paymentForm.amountDue) {
  ...
  const handleUpdatePayment = async () => {
    try {
      const res = await axios.put(`/api/fees/payments/${editingPayment.id}`, paymentForm)
  ...
  const handleApproveResult = async (id) => {
    try {
      const res = await axios.put(`/api/results/${id}`, { status: 'approved' })
  ...
  const handleAdminEditResult = (result) => {
    setAdminEditResult(result)
    setAdminEditScores(result.scores)
    setAdminEditRemarks(result.remarks || '')
  }

  const handleAdminSaveEdit = async () => {
    try {
      const res = await axios.put(`/api/results/${adminEditResult.id}`, {
        scores: adminEditScores,
        remarks: adminEditRemarks,
        status: adminEditResult.status
      })
  ...
  const handleEditStudent = async () => {
    try {
      const res = await axios.put(`/api/students/${editStudent.id}`, editStudent)
                ...
                {['All', 'pending', 'approved', 'rejected'].map(status => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-bold transition-colors capitalize ${
                      admissionFilter === status ? 'bg-[#1a3c6e] text-white' : 'bg-white text-[#1a3c6e] border border-[#1a3c6e] hover:bg-[#1a3c6e] hover:text-white'
                    }`}
                  >
                    {status}
                      ...
                      <div className="flex items-start justify-between flex-wrap gap-4">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 rounded-full overflow-hidden bg-gray-200 border-2 border-[#1a3c6e]">
                            {app.photo ? (
                              <img src={`${app.photo}`} alt={app.firstName} className="w-full h-full object-cover" />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center text-[#1a3c6e] font-bold text-xl">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button
                            ...
                            className="bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-4 py-2 rounded-lg text-sm transition-colors flex items-center gap-1"
                          >
                            <Eye size={14} /> View
                          </button>
                          {app.status === 'pending' && (
                            <>
                              ...
                              <button
                                ...
                                className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold px-4 py-2 rounded-lg text-sm transition-colors"
                              >
                                Reject
                              </button>
                            </>
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                    <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
                      ...
                      <button onClick={() => setViewingApplication(null)} className="hover:text-[#d4a017]">
                        <X size={24} />
                      </button>
                    </div>
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-6 mb-6">
                        <div className="w-24 h-24 rounded-xl overflow-hidden bg-gray-200 border-4 border-[#1a3c6e]">
                          {viewingApplication.photo ? (
                            <img src={`${viewingApplication.photo}`} alt={viewingApplication.firstName} className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <div>
                          ...
                          <span className={`inline-block text-xs font-bold px-3 py-1 rounded-full mt-2 ${
                            viewingApplication.status === 'approved' ? 'bg-green-100 text-green-700' :
                      ...
                      {[
                        { title: "Learner's Data", fields: [
                          ['Date of Birth', viewingApplication.dateOfBirth],
                          ['Gender', viewingApplication.gender],
                          ['Age', viewingApplication.age],
                          ['Place of Birth', viewingApplication.placeOfBirth],
                          ['Hometown', viewingApplication.hometown],
                          ['Mother Tongue', viewingApplication.motherTongue],
                          ['Religion', viewingApplication.religion],
                          ['Grade Level', viewingApplication.gradeLevel],
                          ['Previous School', viewingApplication.previousSchool],
                        ]},
                        { title: 'Family Data', fields: [
                          ['Parent Name', viewingApplication.parentName],
                          ['Parent Email', viewingApplication.parentEmail],
                          ['Parent Phone', viewingApplication.parentPhone],
                          ['Parent Occupation', viewingApplication.parentOccupation],
                          ['Secondary Contact', viewingApplication.secondaryContactName],
                          ['Secondary Phone', viewingApplication.secondaryContactPhone],
                        ]},
                        { title: "Father's Info", fields: [
                          ['Name', viewingApplication.fatherName],
                          ['Phone', viewingApplication.fatherPhone],
                          ['Occupation', viewingApplication.fatherOccupation],
                          ['Education', viewingApplication.fatherEducation],
                          ['Email', viewingApplication.fatherEmail],
                        ]},
                        { title: "Mother's Info", fields: [
                          ['Name', viewingApplication.motherName],
                          ['Phone', viewingApplication.motherPhone],
                          ['Occupation', viewingApplication.motherOccupation],
                          ['Education', viewingApplication.motherEducation],
                          ['Email', viewingApplication.motherEmail],
                        ]},
                        { title: 'Emergency Contact', fields: [
                          ['Name', viewingApplication.emergencyName],
                          ['Relationship', viewingApplication.emergencyRelationship],
                          ['Phone', viewingApplication.emergencyPhone],
                          ['WhatsApp', viewingApplication.emergencyWhatsapp],
                          ['Email', viewingApplication.emergencyEmail],
                        ]},
                        { title: 'Medical & Other', fields: [
                          ['Medical Conditions', viewingApplication.medicalConditions],
                          ['Allergies', viewingApplication.allergies],
                          ['Special Needs', viewingApplication.specialNeeds],
                          ['Doctor', viewingApplication.doctorName],
                          ['Hospital', viewingApplication.hospitalName],
                          ['Lives With', viewingApplication.livesWith],
                          ['Languages', [viewingApplication.language1, viewingApplication.language2, viewingApplication.language3].filter(Boolean).join(', ')],
                        ]},
                      ].map((section, si) => (
                      ...
                      <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          {[
                            { label: 'NHIS Card - Front', url: viewingApplication.nhisFront },
                            { label: 'NHIS Card - Back', url: viewingApplication.nhisBack },
                            { label: 'Ghana Card - Front', url: viewingApplication.ghanaFront },
                            { label: 'Ghana Card - Back', url: viewingApplication.ghanaBack },
                          ].map((doc, di) => (
                            <div key={di} className="bg-gray-50 rounded-lg p-3">
                              <p className="text-xs font-bold text-[#1a3c6e] mb-2">{doc.label}</p>
                              {doc.url ? (
                        ...
                        {viewingApplication.signedBooklet && (
                          <div className="mt-4 bg-blue-50 rounded-lg p-4">
                            <p className="text-sm font-bold text-[#1a3c6e] mb-2">Signed Admission Booklet</p>
                              ...
                              <button
                              onClick={() => window.open(`${viewingApplication.signedBooklet}`, '_blank')}
                              className="inline-block bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-4 py-2 rounded-lg text-sm transition-colors"
                            >
                      ...
                      <div className="flex gap-3 flex-wrap">
                        {viewingApplication.status === 'pending' && (
                          <>
                            <button
                              onClick={() => { handleApproveApplication(viewingApplication.id); setViewingApplication(null) }}
                              className="flex-1 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-xl transition-colors"
                            >
                              Approve & Add to Learners
                            </button>
                            <button
                              onClick={() => { handleRejectApplication(viewingApplication.id); setViewingApplication(null) }}
              ...
              <div className="flex gap-3 mb-6">
                {['fee-structure', 'payments'].map(tab => (
                  <button
                    ...
                    className={`px-6 py-2 rounded-full text-sm font-bold transition-colors capitalize ${
                      financeTab === tab ? 'bg-[#1a3c6e] text-white' : 'bg-white text-[#1a3c6e] border border-[#1a3c6e]'
                    }`}
                  >
              ...
              {financeTab === 'fee-structure' && (
                <div>
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
                      ...
                      {['Nursery', 'Reception', 'Year 1', 'Year 2', 'Year 3', 'Year 4', 'Year 5', 'Year 6'].map(grade => (
                        <div key={grade} className="bg-gray-50 rounded-xl p-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              type="number"
                              value={feeStructures[grade] || ''}
                              onChange={e => setFeeStructures({ ...feeStructures, [grade]: e.target.value })}
                              placeholder="0.00"
                              className="flex-1 px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm"
                            />
                          </div>
                        </div>
                      ))}
                    ...
                    <button
                      onClick={handleSaveFeeStructures}
                      className="mt-6 bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-8 py-3 rounded-xl transition-colors"
                    >
                      Save Fee Structure
                    </button>
                    {feeSaved && (
                      <span className="ml-4 text-green-600 text-sm font-bold">✓ Saved successfully!</span>
                    )}
                  </div>

                  {/* Summary Cards */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div className="bg-[#1a3c6e] text-white rounded-2xl p-6 shadow-md">
                    ...
                    <div className="bg-[#0f6e56] text-white rounded-2xl p-6 shadow-md">
                      <p className="text-green-200 text-sm mb-1">Total Collected</p>
                      <p className="text-3xl font-bold">GH₵ {feePayments.reduce((acc, p) => acc + p.amountPaid, 0).toFixed(2)}</p>
                    </div>
                    <div className="bg-[#d4a017] text-[#1a3c6e] rounded-2xl p-6 shadow-md">
                      <p className="text-[#1a3c6e]/70 text-sm mb-1">Total Outstanding</p>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 mb-6">
                    {['All', 'Nursery', 'Reception', 'Year 1', 'Year 2', 'Year 3', 'Year 4', 'Year 5', 'Year 6'].map(cls => (
                      <button
                        key={cls}
                        onClick={() => setFeeClassFilter(cls)}
                        className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${
                          feeClassFilter === cls ? 'bg-[#1a3c6e] text-white' : 'bg-white text-[#1a3c6e] border border-[#1a3c6e] hover:bg-[#1a3c6e] hover:text-white'
                        }`}
                      >
                        {cls}
                      </button>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filteredFeePayments.length === 0 ? (
                          <tr>
                            <td colSpan="8" className="px-6 py-8 text-center text-gray-400">No payments recorded yet.</td>
                          </tr>
                        ) : (
                            ...
                            <tr key={payment.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                              ...
                              <td className="px-6 py-4">
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => { setEditingPayment(payment); setPaymentForm({ amountPaid: payment.amountPaid, notes: payment.notes || '' }) }}
                                    className="bg-[#d4a017] hover:bg-[#f0c040] text-[#1a3c6e] text-xs font-bold px-3 py-1.5 rounded-lg"
                                  >
                                    Edit
                                  </button>
                                  <button
                  ...
                  <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
                    ...
                    <div className="p-6 space-y-4">
                      <div>
                        ...
                        <select value={paymentForm.studentId} onChange={e => { setPaymentForm({ ...paymentForm, studentId: e.target.value }); const s = students.find(s => s.id === parseInt(e.target.value)); if (s) setPaymentForm(prev => ({ ...prev, studentId: e.target.value, amountDue: feeStructures[s.gradeLevel] || '' })) }} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700">
              ...
              <div className="flex flex-wrap gap-3 mb-6">
                {['All', 'pending', 'approved', 'rejected'].map(status => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-bold transition-colors capitalize ${
                      resultFilter === status ? 'bg-[#1a3c6e] text-white' : 'bg-white text-[#1a3c6e] border border-[#1a3c6e] hover:bg-[#1a3c6e] hover:text-white'
                    }`}
                  >
                    {status}
                    <span className="ml-2 bg-[#d4a017] text-[#1a3c6e] text-xs font-bold px-2 py-0.5 rounded-full">
                      {status === 'All' ? results.length : results.filter(r => r.status === status).length}
                    </span>
                  </button>
                ))}
              </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 border-2 border-[#1a3c6e]">
                            {result.student?.photo ? (
                              <img src={`${result.student.photo}`} alt={result.student.firstName} className="w-full h-full object-cover" />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center text-[#1a3c6e] font-bold">
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                    <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
                      ...
                      <button onClick={() => setAdminEditResult(null)} className="hover:text-[#d4a017]">
                        <X size={24} />
                      </button>
                    </div>
                    <div className="p-6 space-y-4">
                        ...
                        return (
                          <div key={subject} className="bg-gray-50 rounded-xl p-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {[
                                { key: 'classScore', label: 'Class(10)' },
                                { key: 'cat1', label: 'CAT1(20)' },
                                { key: 'cat2', label: 'CAT2(20)' },
                                { key: 'exam', label: 'Exam(100)' },
                              ].map(field => (
                                <div key={field.key}>
                                  ...
                                  <input
                                    ...
                                    value={s[field.key] || ''}
                                    onChange={e => setAdminEditScores(prev => ({
                                      ...prev,
                                      [subject]: { ...prev[subject], [field.key]: e.target.value }
                                    }))}
                                    className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm"
                                  />
                                </div>
                              ))}
                            </div>
                      ...
                      <div className="flex gap-3">
                        ...
                        <button
                          onClick={() => setAdminEditResult(null)}
                          className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl transition-colors"
                        >
                          Cancel
                        </button>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-6 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
                >
              ...
              {newToken && (
                <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-bold text-green-700">New Token Generated!</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white rounded-xl p-4 border border-green-200">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => handleCopy(newToken.serialNumber, 'serial')} className="text-gray-400 hover:text-[#1a3c6e]">
                          {copiedId === 'serial' ? <CheckCircle size={16} className="text-green-600" /> : <Copy size={16} />}
                        </button>
                      </div>
                    </div>
                    <div className="bg-white rounded-xl p-4 border border-green-200">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => handleCopy(newToken.pin, 'pin')} className="text-gray-400 hover:text-[#1a3c6e]">
                          {copiedId === 'pin' ? <CheckCircle size={16} className="text-green-600" /> : <Copy size={16} />}
                        </button>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => setNewToken(null)} className="mt-4 text-sm text-gray-500 hover:text-gray-700">
                    Dismiss
                  </button>
                </div>
              )}
                    ...
                    {tokens.length === 0 ? (
                      ...
                      tokens.map((token, index) => (
                        <tr key={token.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                              token.used ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                            }`}>
          ...
          {activeMenu === 'gallery' && (
            <div>
                ...
                <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 mb-6">
                        ...
                        <div className="grid grid-cols-4 gap-3 mt-3">
                          {galleryPreviews.map((preview, index) => (
                            <div key={index} className="relative">
                              ...
                              <span className="absolute top-1 right-1 bg-[#1a3c6e] text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
                                {index + 1}
                              </span>
                            </div>
                          ))}
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-8 py-3 rounded-xl transition-colors disabled:opacity-50"
                    >
                      {galleryLoading ? 'Uploading...' : 'Upload Gallery Item'}
                    </button>
                    <button
                      onClick={() => { setShowAddGallery(false); setGalleryPreviews([]); setGalleryImages([]) }}
              ...
              {galleryItems.length === 0 ? (
                <div className="bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">
                  No gallery items yet. Click "Add Gallery Item" to upload photos.
                </div>
              ) : (
                  ...
                  {galleryItems.map(item => (
                    <div key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                      <div
                        ...
                        onClick={() => { setViewingGallery(item); setActiveGalleryImage(0) }}
                      >
                        <img
                          src={`${item.images[0]}`}
                          alt={item.title}
                        ...
                        <span className="absolute top-2 left-2 bg-[#d4a017] text-[#1a3c6e] text-xs font-bold px-3 py-1 rounded-full">
                          {item.category}
                        </span>
                      </div>
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={(e) => { e.stopPropagation(); setEditingGallery(item); setGalleryForm({ title: item.title, description: item.description || '', category: item.category }) }}
                              className="bg-[#d4a017] hover:bg-[#f0c040] text-[#1a3c6e] p-1.5 rounded-lg transition-colors text-xs font-bold px-3"
                            >
                              Edit
                            </button>
                            <button
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                    <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
                      ...
                      <button onClick={() => setEditingGallery(null)} className="hover:text-[#d4a017]">
                        <X size={24} />
                      </button>
                    </div>
                    <div className="p-6 space-y-4">
                      ...
                      <div>
                        ...
                        {galleryPreviews.length > 0 && (
                          <div className="grid grid-cols-4 gap-2 mt-3">
                            {galleryPreviews.map((preview, index) => (
                              <img key={index} src={preview} alt={`Preview ${index + 1}`} className="w-full h-16 object-cover rounded-lg" />
                            ))}
                          </div>
                        )}
                        {galleryPreviews.length === 0 && (
                          <div className="grid grid-cols-4 gap-2 mt-3">
                            {editingGallery.images.map((img, index) => (
                              <img key={index} src={`${img}`} alt={`Current ${index + 1}`} className="w-full h-16 object-cover rounded-lg opacity-60" />
                            ))}
                          </div>
                        )}
                      </div>
                      <div className="flex gap-3">
                        ...
                        <button
                          onClick={() => { setEditingGallery(null); setGalleryPreviews([]); setGalleryImages([]) }}
                          className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl transition-colors"
                        >
                          Cancel
                        </button>
                ...
                <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
                  <div className="max-w-4xl w-full">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="text-white font-bold text-xl">{viewingGallery.title}</h3>
                        <p className="text-gray-400 text-sm">{viewingGallery.category} | {new Date(viewingGallery.createdAt).toLocaleDateString()}</p>
                      </div>
                      <button onClick={() => setViewingGallery(null)} className="text-white hover:text-[#d4a017] transition-colors">
                        <X size={28} />
                      </button>
                    </div>
                    ...
                    <div className="relative mb-4">
                      <img
                        src={`${viewingGallery.images[activeGalleryImage]}`}
                      ...
                      {viewingGallery.images.length > 1 && (
                        <>
                          <button
                            onClick={() => setActiveGalleryImage(prev => prev === 0 ? viewingGallery.images.length - 1 : prev - 1)}
                          ...
                          <button
                            onClick={() => setActiveGalleryImage(prev => prev === viewingGallery.images.length - 1 ? 0 : prev + 1)}
                      ...
                      <span className="absolute bottom-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded-full">
                        {activeGalleryImage + 1} / {viewingGallery.images.length}
                      </span>
                    </div>
                        ...
                        {viewingGallery.images.map((img, index) => (
                          <img
                            ...
                            className={`w-16 h-16 object-cover rounded-lg cursor-pointer shrink-0 transition-all ${
                              activeGalleryImage === index ? 'ring-2 ring-[#d4a017]' : 'opacity-60 hover:opacity-100'
                            }`}
                          />
                        ))}
                      </div>
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                    <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
                      ...
                      <button onClick={() => { setShowAddStaff(false); setEditingStaff(null); setStaffPhotoPreview(null); setStaffPhoto(null) }} className="hover:text-[#d4a017]">
                        <X size={24} />
                      </button>
                    </div>
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-6 mb-6">
                        <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-200 border-4 border-[#1a3c6e]">
                          {staffPhotoPreview ? (
                            <img src={staffPhotoPreview} alt="Preview" className="w-full h-full object-cover" />
                          ) : editingStaff?.photo ? (
                            <img src={`${editingStaff.photo}`} alt="Current" className="w-full h-full object-cover" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center text-[#1a3c6e] font-bold text-2xl">
                              {staffForm.name?.charAt(0) || '?'}
                            </div>
                          )}
                        </div>
                        <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Full Name <span className="text-red-500">*</span></label>
                          <input type="text" value={staffForm.name} onChange={e => setStaffForm({ ...staffForm, name: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Role <span className="text-red-500">*</span></label>
                          <input type="text" value={staffForm.role} onChange={e => setStaffForm({ ...staffForm, role: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Department</label>
                          <input type="text" value={staffForm.department} onChange={e => setStaffForm({ ...staffForm, department: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Category</label>
                        ...
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Subject (for teachers)</label>
                          <input type="text" value={staffForm.subject} onChange={e => setStaffForm({ ...staffForm, subject: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Email</label>
                          <input type="email" value={staffForm.email} onChange={e => setStaffForm({ ...staffForm, email: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Phone</label>
                          <input type="text" value={staffForm.phone} onChange={e => setStaffForm({ ...staffForm, phone: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700" />
                        </div>
                        <div className="md:col-span-2">
                      ...
                      <div className="flex gap-4 mt-6">
                        <button
                          ...
                          className="flex-1 bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold py-3 rounded-xl transition-colors disabled:opacity-50"
                        >
                          {staffLoading ? 'Saving...' : editingStaff ? 'Save Changes' : 'Add Staff Member'}
                        </button>
                        <button
                          onClick={() => { setShowAddStaff(false); setEditingStaff(null); setStaffPhotoPreview(null); setStaffPhoto(null) }}
                          className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl transition-colors"
                        >
                          Cancel
                        </button>
                    ...
                    <div key={member.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                      <div className="relative">
                        {member.photo ? (
                          ...
                          <div className="w-full h-48 bg-[#1a3c6e] flex items-center justify-center">
                            <span className="text-[#d4a017] text-4xl font-bold">{member.name?.charAt(0)}</span>
                          </div>
                        )}
                        <span className={`absolute top-2 left-2 text-xs font-bold px-3 py-1 rounded-full ${
                          member.category === 'leadership' ? 'bg-[#d4a017] text-[#1a3c6e]' :
                          member.category === 'teaching' ? 'bg-[#1a3c6e] text-white' :
                          ...
                          <button
                            onClick={() => {
                              ...
                              setStaffForm({
                                ...
                                subject: member.subject || '', bio: member.bio || '', email: member.email || '',
                                phone: member.phone || '', category: member.category
                              })
                              setStaffPhotoPreview(null)
                              setStaffPhoto(null)
                            }}
                            className="flex-1 bg-[#d4a017] hover:bg-[#f0c040] text-[#1a3c6e] font-bold py-2 rounded-lg text-sm transition-colors"
                          >
                            Edit
                          </button>
                          <button
            ...
            <div>
                ...
                {['All', 'news', 'event'].map(f => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-full text-sm font-bold transition-colors capitalize ${
                      newsFilter === f ? 'bg-[#1a3c6e] text-white' : 'bg-white text-[#1a3c6e] border border-[#1a3c6e] hover:bg-[#1a3c6e] hover:text-white'
                    }`}
                  >
                    {f === 'All' ? 'All' : f === 'news' ? 'News' : 'Events'}
                    <span className="ml-2 bg-[#d4a017] text-[#1a3c6e] text-xs font-bold px-2 py-0.5 rounded-full">
                      {f === 'All' ? newsItems.length : newsItems.filter(n => n.type === f).length}
                    </span>
                  </button>
                ))}
              </div>
              ...
              {(showAddNews || editingNews) && (
                ...
                <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
                  ...
                  <button onClick={() => { setShowAddNews(false); setEditingNews(null); setNewsForm({ title: '', content: '', category: 'General', type: 'news', videoUrl: '', eventDate: '', venue: '' }); setNewsImages([]); setNewsPreviews([]) }} className="hover:text-[#d4a017]">
                    <X size={24} />
                  </button>
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-bold text-[#1a3c6e] mb-6 hidden">
                    {editingNews ? 'Edit Item' : 'Add News / Event'}
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    {newsForm.type === 'event' && (
                      <>
                        <div>
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Event Date</label>
                          <input
                            type="date"
                        ...
                        <div className="md:col-span-2">
                          <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Venue</label>
                          <input
                            type="text"
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Content</label>
                      <textarea
                        value={newsForm.content}
                        onChange={e => setNewsForm({ ...newsForm, content: e.target.value })}
                        rows={5}
                        placeholder="Write the news or event details here..."
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#1a3c6e] mb-2">Video URL (optional)</label>
                      <input
                        type="text"
                    ...
                    <div>
                      ...
                      {newsPreviews.length > 0 && (
                        <div className="grid grid-cols-4 gap-2 mt-3">
                          {newsPreviews.map((preview, index) => (
                            <img key={index} src={preview} alt={`Preview ${index + 1}`} className="w-full h-16 object-cover rounded-lg" />
                          ))}
                        </div>
                      )}
                  ...
                  <div className="flex gap-4 mt-6">
                    <button
                      ...
                      className="bg-[#1a3c6e] hover:bg-[#2a5298] text-white font-bold px-8 py-3 rounded-xl transition-colors disabled:opacity-50"
                    >
                      {newsLoading ? 'Saving...' : editingNews ? 'Save Changes' : 'Publish'}
                    </button>
                    <button
                      onClick={() => {
                        setShowAddNews(false)
              ...
              {filteredNews.length === 0 ? (
                <div className="bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">
                  No news or events yet. Click "Add News / Event" to create one.
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredNews.map(item => (
                    <div key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                      ...
                      {item.videoUrl && !item.images?.length && (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="bg-[#d4a017] hover:bg-[#f0c040] text-[#1a3c6e] text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
                            >
                              Edit
                            </button>
                            <button
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="bg-[#1a3c6e] text-white p-6 rounded-t-2xl flex items-center justify-between">
              ...
              <button onClick={() => { setSelectedStudent(null); setEditMode(false) }} className="hover:text-[#d4a017] transition-colors">
                <X size={24} />
              </button>
            </div>
            <div className="p-6">
              ...
              {!editMode ? (
                <div>
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => { setEditMode(true); setEditStudent({ ...selectedStudent }) }}
              ...
              /* Edit Mode */
                <div>
                  <div className="grid grid-cols-1 gap-4 mb-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-[#1a3c6e] mb-1">First Name</label>
                        <input type="text" value={editStudent.firstName} onChange={e => setEditStudent({ ...editStudent, firstName: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-[#1a3c6e] mb-1">Last Name</label>
                        <input type="text" value={editStudent.lastName} onChange={e => setEditStudent({ ...editStudent, lastName: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-[#1a3c6e] mb-1">Date of Birth</label>
                        <input type="date" value={editStudent.dateOfBirth} onChange={e => setEditStudent({ ...editStudent, dateOfBirth: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                      </div>
                      <div>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-[#1a3c6e] mb-1">Parent Name</label>
                      <input type="text" value={editStudent.parentName} onChange={e => setEditStudent({ ...editStudent, parentName: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#1a3c6e] mb-1">Parent Email</label>
                      <input type="email" value={editStudent.parentEmail} onChange={e => setEditStudent({ ...editStudent, parentEmail: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#1a3c6e] mb-1">Parent Phone</label>
                      <input type="text" value={editStudent.parentPhone} onChange={e => setEditStudent({ ...editStudent, parentPhone: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#1a3c6e] mb-1">Address</label>
                      <input type="text" value={editStudent.address} onChange={e => setEditStudent({ ...editStudent, address: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1a3c6e] text-gray-700 text-sm" />
                    </div>
                    <div>
