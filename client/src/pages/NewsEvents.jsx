  ...
  return (
    <div>
      ...
      {upcomingEvents.length > 0 && (
        <section className="bg-sky-500 py-4">
          <div className="max-w-7xl mx-auto px-4 flex items-center gap-4 overflow-x-auto">
            <span className="text-sky-700 font-bold text-sm shrink-0">📅 Upcoming:</span>
            {upcomingEvents.map(event => (
              <div key={event.id} className="flex items-center gap-2 shrink-0">
                <span className="text-sky-700 text-sm font-bold">{event.title}</span>
                <span className="text-sky-700/70 text-xs">— {new Date(event.eventDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                {event.venue && <span className="text-sky-700/70 text-xs">@ {event.venue}</span>}
                <span className="text-sky-700/40 mx-2">|</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="relative max-w-2xl mx-auto mb-6">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-xl border border-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-600 text-gray-700 text-lg"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${
                    activeCategory === cat
                      ...
                      : 'bg-white text-sky-700 border border-sky-600 hover:bg-sky-600 hover:text-white'
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${viewMode === 'grid' ? 'bg-sky-600 text-white' : 'bg-white text-sky-700 border border-sky-600'}`}
              >
                Grid
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${viewMode === 'list' ? 'bg-sky-600 text-white' : 'bg-white text-sky-700 border border-sky-600'}`}
              >
                List
              </button>
            </div>
              ...
              {filtered.map(item => (
                ...
                >
                  ...
                  {item.images && item.images.length > 0 ? (
                    ...
                    <div className="w-full h-52 bg-sky-600 flex items-center justify-center">
                      <span className="text-sky-600 text-5xl font-bold font-serif">GI</span>
                    </div>
                  )}
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                        item.type === 'event' ? 'bg-sky-500 text-sky-700' : 'bg-sky-600 text-white'
                      }`}>
                        {item.type === 'event' ? '📅 Event' : '📰 News'}
                      </span>
                      <span className="text-xs text-gray-400">{item.category}</span>
                    </div>
                    <h3 className="font-bold text-sky-700 text-lg mb-2 line-clamp-2">{item.title}</h3>
                    ...
                    {item.type === 'event' && item.eventDate && (
                      <div className="bg-blue-50 rounded-xl px-4 py-3 mb-3 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-xs font-bold text-sky-700">{new Date(item.eventDate).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
                        </div>
                        {item.venue && (
                          <div className="flex items-center gap-2">
              ...
              {filtered.map(item => (
                ...
                >
                  {item.images && item.images.length > 0 ? (
                    ...
                    <div className="w-32 h-24 bg-sky-600 rounded-xl flex items-center justify-center shrink-0">
                      <span className="text-sky-600 font-bold text-xl">GI</span>
                    </div>
                  )}
                  <div className="flex-1">
                    ...
                    <h3 className="font-bold text-sky-700 text-lg mb-1">{item.title}</h3>
                    <p className="text-sm text-gray-500 line-clamp-2 mb-2">{item.content}</p>
                    {item.type === 'event' && item.eventDate && (
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-1">
                          <Calendar size={12} className="text-sky-700" />
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="bg-sky-600 text-white p-6 rounded-t-2xl flex items-center justify-between">
              <div>
                <span className={`text-xs font-bold px-3 py-1 rounded-full mr-2 ${
                  viewingItem.type === 'event' ? 'bg-sky-500 text-sky-700' : 'bg-white/20 text-white'
                }`}>
                  {viewingItem.type === 'event' ? '📅 Event' : '📰 News'}
                </span>
                <span className="text-sky-100 text-xs">{viewingItem.category}</span>
              </div>
              <button onClick={() => setViewingItem(null)} className="hover:text-sky-600 transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6">
              ...
              {/* Title */}
              <h2 className="text-2xl font-bold font-serif text-sky-700 mb-4">{viewingItem.title}</h2>
              ...
              {viewingItem.type === 'event' && viewingItem.eventDate && (
                <div className="bg-blue-50 rounded-xl p-4 mb-4 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-bold text-sky-700">{new Date(viewingItem.eventDate).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
                  </div>
                  {viewingItem.venue && (
                    <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="w-full bg-sky-600 hover:bg-sky-400 text-white font-bold py-3 rounded-xl transition-colors mt-6"
              >
                Close
              </button>
            </div>
