...
export default function Staff() {
    ...
    >
      <div className="relative">
        {member.photo ? (
          ...
          <div className="w-full h-56 bg-sky-600 flex items-center justify-center">
            <span className="text-sky-600 text-5xl font-bold">{member.name?.charAt(0)}</span>
          </div>
        )}
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
          <span className="inline-block bg-sky-500 text-sky-700 text-xs font-bold px-3 py-1 rounded-full">
            {member.department}
          </span>
        </div>
      </div>
      <div className="p-5">
        <h3 className="font-bold text-sky-700 text-lg mb-1">{member.name}</h3>
        <p className="text-sm text-gray-500 mb-1">{member.role}</p>
        {member.subject && <p className="text-xs text-sky-600 font-bold mb-3">{member.subject}</p>}
  ...
  const SectionTitle = ({ title, subtitle }) => (
    <div className="text-center mb-12">
      <span className="inline-block bg-sky-500 text-sky-700 text-sm font-bold px-4 py-1 rounded-full mb-4">{subtitle}</span>
      <h2 className="text-3xl md:text-4xl font-bold font-serif text-sky-700">{title}</h2>
    </div>
  )
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="bg-sky-600 text-white p-6 rounded-t-2xl flex items-center justify-between">
              ...
              <button onClick={() => setSelectedStaff(null)} className="hover:text-sky-600"><X size={24} /></button>
            </div>
            <div className="p-6">
              <div className="flex items-center gap-6 mb-6">
                <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-200 border-4 border-sky-600 shrink-0">
                  {selectedStaff.photo ? (
                    <img src={`${selectedStaff.photo}`} alt={selectedStaff.name} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-sky-700 font-bold text-2xl">
                      {selectedStaff.name?.charAt(0)}
                    </div>
                  )}
                </div>
                <div>
                  <h3 className="text-xl font-bold text-sky-700">{selectedStaff.name}</h3>
                  <p className="text-gray-500">{selectedStaff.role}</p>
                  <span className="inline-block bg-sky-500 text-sky-700 text-xs font-bold px-3 py-1 rounded-full mt-1">{selectedStaff.department}</span>
                  {selectedStaff.subject && <p className="text-xs text-sky-700 font-bold mt-1">Teaches: {selectedStaff.subject}</p>}
                </div>
              </div>
              {selectedStaff.bio && (
