...
export default function Staff() {
    ...
    >
      <div className="relative">
        {member.photo ? (
          ...
          <div className="w-full h-56 bg-[#8a2be2] flex items-center justify-center">
            <span className="text-white text-5xl font-bold">{member.name?.charAt(0)}</span>
          </div>
        )}
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
          <span className="inline-block bg-yellow-400 text-slate-900 text-xs font-bold px-3 py-1 rounded-full">
            {member.department}
          </span>
        </div>
      </div>
      <div className="p-5">
        ...
        {member.subject && <p className="text-xs text-[#8a2be2] font-bold mb-3">{member.subject}</p>}
  ...
  const SectionTitle = ({ title, subtitle }) => (
    <div className="text-center mb-12">
      <span className="inline-block bg-[#8a2be2] text-white text-sm font-bold px-4 py-1 rounded-full mb-4">{subtitle}</span>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="bg-[#8a2be2] text-white p-6 rounded-t-2xl flex items-center justify-between">
              ...
              <button onClick={() => setSelectedStaff(null)} className="hover:text-yellow-300 transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-6 mb-6">
                <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-200 border-4 border-[#8a2be2] shrink-0">
                  {selectedStaff.photo ? (
                    <img src={`${selectedStaff.photo}`} alt={selectedStaff.name} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-[#8a2be2] text-white font-bold text-3xl">
                      {selectedStaff.name?.charAt(0)}
                    </div>
                  )}
                </div>
                <div>
                  <h3 className="text-xl font-bold text-slate-800">{selectedStaff.name}</h3>
                  <p className="text-gray-500">{selectedStaff.role}</p>
                  <span className="inline-block bg-yellow-400 text-slate-900 text-xs font-bold px-3 py-1 rounded-full mt-1">
                    {selectedStaff.department}
                  </span>
                  {selectedStaff.subject && <p className="text-xs text-[#8a2be2] font-bold mt-1">Teaches: {selectedStaff.subject}</p>}
                </div>
              </div>
              ...
              <div className="space-y-2">
                {selectedStaff.email && (
                  <div className="flex items-center gap-3">
                    <Mail size={16} className="text-[#8a2be2]" />
                    <span className="text-sm text-gray-600">{selectedStaff.email}</span>
                  </div>
                )}
                {selectedStaff.phone && (
                  <div className="flex items-center gap-3">
                    <Phone size={16} className="text-[#8a2be2]" />
